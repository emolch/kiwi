! 
!    Copyright 2011 Sebastian Heimann, Simone Cesca
! 
...
module source_mt_eikonal
    ...
    integer, public, parameter :: n_source_params_mt_eikonal = 20
    integer, public, parameter :: n_grid_dims_mt_eikonal = 2
    
    public psm_to_tdsm_mt_eikonal, psm_write_info_file_mt_eikonal, psm_set_mt_eikonal
    public psm_get_param_name_mt_eikonal, psm_get_param_unit_mt_eikonal, psm_get_param_id_mt_eikonal
    public psm_cleanup_mt_eikonal
    
    private psm_to_tdsm_size_mt_eikonal, psm_to_tdsm_table_mt_eikonal
    
    type(varying_string), private, dimension(:), allocatable :: psm_param_names_mt_eikonal, psm_param_units_mt_eikonal
    
  ! these values may be used to hint step sizes and the like
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_norm_mt_eikonal = &
    (/ 1., 10000., 10000., 10000., 7e18, 360., 90., 10000., 10000., 10000.,  360., 10000.,  1., 7e18, &
      7e18, 7e18, 7e18, 7e18, 7e18, 1. /)
    
    real, private, parameter :: big = huge(psm_params_norm_mt_eikonal(1))
    
  ! logical, physical or computational accuracy limits for params
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_min_hard_mt_eikonal = &
    (/ -big, -100000., -100000.,        0., 1., -big, -big, -1e7, -1e7, 0., -1e7, -1e7, 0.1, &
      -7e25, -7e25, -7e25, -7e25, -7e25, -7e25, 0./)
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_max_hard_mt_eikonal = &
    (/ big, 100000.,    100000., 1000000., 7e25, big, big, 1e7, 1e7, 1e7, 1e7, 1e7, 10., 7e25, 7e25, &
       7e25, 7e25, 7e25, 7e25, 10./)
    
  ! these limits give range of realistic, non-redundant or practical params
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_min_soft_mt_eikonal = &
    (/ -20., -10000., -10000., 0., 1., -180., 0., -100000., -100000., 0., -100000., -100000., 0.5, &
       -7e25, -7e25, -7e25, -7e25, -7e25, -7e25, 0./)
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_max_soft_mt_eikonal = &
    (/ 20., 10000., 10000., 150000., 7e25, 180., 90., 100000., 100000., 100000., 100000., 100000., 1.5, & 
       7e25, 7e25, 7e25, 7e25, 7e25, 7e25, 5./)
    
  ! defaults for external use
    real, dimension(n_source_params_mt_eikonal), public, parameter :: psm_params_default_mt_eikonal = &
    (/ 0.,0., 0., 3000.,    1.,    0., 80.,   0., 0., 5000., 0., 0., 0.9, 0., 0., 0., 7e18, 0., 0., 1./)
  ...
  contains
    ...
    subroutine psm_cleanup_mt_eikonal()
    
        integer :: i
        
        if (allocated( psm_param_names_mt_eikonal )) then
            do i=1,n_source_params_mt_eikonal
                call delete( psm_param_names_mt_eikonal(i) )
            end do
            deallocate( psm_param_names_mt_eikonal )
        end if
        if (allocated( psm_param_units_mt_eikonal )) then
            do i=1,n_source_params_mt_eikonal
                call delete( psm_param_units_mt_eikonal(i) )
            end do
            deallocate( psm_param_units_mt_eikonal )
        end if
        
    end subroutine
  
    subroutine psm_init_param_names_mt_eikonal()
    
        if (.not. allocated( psm_param_names_mt_eikonal )) then
            allocate( psm_param_names_mt_eikonal( n_source_params_mt_eikonal ) )
            psm_param_names_mt_eikonal(1) = "time"
            psm_param_names_mt_eikonal(2) = "north-shift"
            psm_param_names_mt_eikonal(3) = "east-shift"
            psm_param_names_mt_eikonal(4) = "depth"
            psm_param_names_mt_eikonal(5) = "moment-factor"
            psm_param_names_mt_eikonal(6) = "strike"
            psm_param_names_mt_eikonal(7) = "dip"
            
            psm_param_names_mt_eikonal(8) = "bord-shift-x"    ! (rightward)
            psm_param_names_mt_eikonal(9) = "bord-shift-y"   ! (downward)
            psm_param_names_mt_eikonal(10) = "bord-radius"
            
            psm_param_names_mt_eikonal(11) = "nukl-shift-x"   ! (rightward)
            psm_param_names_mt_eikonal(12) = "nukl-shift-y"   ! (downward)
            
            psm_param_names_mt_eikonal(13) = "rel-rupture-velocity"
            psm_param_names_mt_eikonal(14) = "mxx"
            psm_param_names_mt_eikonal(15) = "myy"
            psm_param_names_mt_eikonal(16) = "mzz"
            psm_param_names_mt_eikonal(17) = "mxy"
            psm_param_names_mt_eikonal(18) = "mxz"
            psm_param_names_mt_eikonal(19) = "myz"
            psm_param_names_mt_eikonal(20) = "rise-time"
        end if
        
        if (.not. allocated( psm_param_units_mt_eikonal )) then
            allocate( psm_param_units_mt_eikonal( n_source_params_mt_eikonal ) )
            psm_param_units_mt_eikonal(1) = "s"
            psm_param_units_mt_eikonal(2) = "m"
            psm_param_units_mt_eikonal(3) = "m"
            psm_param_units_mt_eikonal(4) = "m"
            psm_param_units_mt_eikonal(5) = "1"
            psm_param_units_mt_eikonal(6) = "degrees"
            psm_param_units_mt_eikonal(7) = "degrees"
            
            psm_param_units_mt_eikonal(8) = "m"
            psm_param_units_mt_eikonal(9) = "m"
            psm_param_units_mt_eikonal(10) = "m"
            
            psm_param_units_mt_eikonal(11) = "m"
            psm_param_units_mt_eikonal(12) = "m"
            
            psm_param_units_mt_eikonal(13) = "1"
            psm_param_units_mt_eikonal(14) = "Nm"
            psm_param_units_mt_eikonal(15) = "Nm"
            psm_param_units_mt_eikonal(16) = "Nm"
            psm_param_units_mt_eikonal(17) = "Nm"
            psm_param_units_mt_eikonal(18) = "Nm"
            psm_param_units_mt_eikonal(19) = "Nm"
            psm_param_units_mt_eikonal(20) = "s"
        end if
        
    end subroutine
  
    subroutine psm_get_param_name_mt_eikonal( iparam, name )
    
        integer, intent(in)                 :: iparam
        type(varying_string), intent(inout) :: name
        
        call psm_init_param_names_mt_eikonal()
        
        name = ""
        if (iparam < 1 .or. n_source_params_mt_eikonal < iparam) return
        name = psm_param_names_mt_eikonal(iparam)
        
    end subroutine
    
    subroutine psm_get_param_unit_mt_eikonal( iparam, unit )
        ...
        unit = ""
        if (iparam < 1 .or. n_source_params_mt_eikonal < iparam) return
        unit = psm_param_units_mt_eikonal(iparam)
        
    end subroutine
   
    subroutine psm_get_param_id_mt_eikonal( name, iparam )
        ...
        iparam = 0
        do i=1, n_source_params_mt_eikonal
            if (name .eq. psm_param_names_mt_eikonal(i)) then
                iparam = i
                return
            end if
        end do
    ...
    subroutine psm_set_mt_eikonal( psm,  params, normalized, only_moment_changed)
        ...
        if (.not. allocated(psm%grid_size) .or. (psm%sourcetype .ne. psm_mt_eikonal)) then
            must_reset_grid = .true.
        end if
        
        call resize( psm%params, 1, n_source_params_mt_eikonal )
        call resize( psm%grid_size, 1, n_grid_dims_mt_eikonal )
        call resize( psm%params_norm, 1, n_source_params_mt_eikonal )
        
        psm%params_norm(:) = psm_params_norm_mt_eikonal(:)
        
        if (must_reset_grid) then
            psm%grid_size = 1
        end if
        
        if (size(params,1) .ne. size(psm%params)) call die("wrong number of source parameters in psm_set_mt_eikonal()")
        ...
        do ip=1,size(psm%params)
            if (ip .ne. 5 .and. ip .ne. 21 .and. psm%params(ip) .ne. new_params(ip)) then
        ...
        psm%risetime = psm%params(20)
        
        call psm_update_dep_params_mt_eikonal( psm )
        
    end subroutine
        
    subroutine psm_update_dep_params_mt_eikonal( psm )
    ...
    subroutine psm_to_tdsm_mt_eikonal( psm, tdsm, shortest_doi, ok )
      ...
      ! automatically determines shape of output grid
        ...
        bord_shift_x = psm%params(8)
        bord_shift_y = psm%params(9)
        bord_radius = psm%params(10)
      
        ok = .true.
      ! determine region of interest
        call psm_borderline_mt_eikonal( psm, bord_shift_x, bord_shift_y, bord_radius, rupture_poly, rupture_poly_rc )
      ...
      ! solve mt_eikonal equation on a fine and rectangular grid
        call polygon_box( rupture_poly_rc, min_rc, max_rc )
        deltagrid = min(100.*shortest_doi/2.,4000.)
        call psm_make_mt_eikonal_grid( psm, min_rc, max_rc, deltagrid, psm%egrid, ok )
        if (.not. ok) return
      ...
      ! determine optimal grid size  
        ...
        call psm_to_tdsm_size_mt_eikonal( psm%egrid%last(1)-psm%egrid%first(1), &
      ...
      ! calculate table of centroids approximating moment tensor density
        call psm_to_tdsm_table_mt_eikonal( psm, psm%cgrid, maxdt, tdsm ) 
    ...
    subroutine psm_borderline_mt_eikonal( psm, bord_shift_x, bord_shift_y, bord_radius, &
    ...
    function psm_initial_point_rc( psm, borderline )
      ...
      ! the polygon.
        ...
        bord_radius = psm%params(10)
        nukl_shift_x = psm%params(11)
        nukl_shift_y = psm%params(12)
    ...
    subroutine psm_make_mt_eikonal_grid( psm, min_rc, max_rc, approx_delta, grid, ok )
        ...
        rel_rupture_velocity = psm%params(13)
    ...
    subroutine psm_to_tdsm_size_mt_eikonal( sizex, sizey, maxdx, maxdy, nx, ny)
    ...
    subroutine psm_to_tdsm_table_mt_eikonal( psm, grid, maxdt, out )
            ...
            do ix=1,grid%ndims(1)
              ...
              ! using a zero risetime here, effect of (constant) risetime applied after synthesis of seismogram
                ...
                do it=1,nt
                    ...
                    out%centroids(id)%m(1) = psm%params(14)*tweights(it) * grid%weights(ix,iy)
                    out%centroids(id)%m(2) = psm%params(15)*tweights(it) * grid%weights(ix,iy)
                    out%centroids(id)%m(3) = psm%params(16)*tweights(it) * grid%weights(ix,iy)
                    out%centroids(id)%m(4) = psm%params(17)*tweights(it) * grid%weights(ix,iy)
                    out%centroids(id)%m(5) = psm%params(18)*tweights(it) * grid%weights(ix,iy)
                    out%centroids(id)%m(6) = psm%params(19)*tweights(it) * grid%weights(ix,iy)
                    id = id+1
                end do
            end do
        end do
    ...
    subroutine psm_write_info_file_mt_eikonal( psm, fn )
