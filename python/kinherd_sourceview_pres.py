...
class MyWindow(QMainWindow):
    ...
    def __init__(self, clargs, *args):
        ...
        if (len(clargs) != 7): 
            sys.exit("usage: kinherd_sourceview_pres.py database effective-dt origin-lat origin-lon receiver-lat receiver-lon")
        
        (pn, gfdb, effective_dt, origin_lat, origin_lon, rec_lat, rec_lon) = clargs
        ...
        receiverfile = os.path.join(self.minimizer.tempdir,'receivers')
        f = open(receiverfile,'w')
        f.write( '%g %g ned\n' % (float(rec_lat), float(rec_lon)) )
        f.close()
        
        self.minimizer.do_set_database( gfdb )
        self.minimizer.do_set_effective_dt(effective_dt )
        self.minimizer.do_set_source_location( origin_lat, origin_lon, 0. )
        self.minimizer.do_set_receivers( receiverfile )
    ...
    def calculate(self,data):
        ...
        self.minimizer.do_output_seismograms( self.minimizer.tempfilebase, "table", "synthetics", "plain")

        
        minimizer.table_to_bin( self.minimizer.tempfilebase+'-dsm.table', self.minimizer.tempfilebase+'-dsm.bin' )
        minimizer.table_to_bin( self.minimizer.tempfilebase+'-1-n.table', self.minimizer.tempfilebase+'-1-n.bin' )
        minimizer.table_to_bin( self.minimizer.tempfilebase+'-1-e.table', self.minimizer.tempfilebase+'-1-e.bin' )
        minimizer.table_to_bin( self.minimizer.tempfilebase+'-1-d.table', self.minimizer.tempfilebase+'-1-d.bin' )
