  ...
  contains
    ...
    subroutine chunk_create( c )
      ...
      ! create index dataset for faster access to individual traces
        ...
        call h5gcreate_f(file,"gf",group,error,int(c%nxc/c%nipx*(floor(log10(real(c%nxc/c%nipx)))+2),4))
        if (error /= 0) &
    ...
    subroutine chunk_save_trace( db, c, ixc, iz, ig, data )
      ...
      ! open/create group for the dataset
        ...
        call h5_opencreategroup( c%file, char(gloc), group_dist, e(1), int(c%nz/c%nipz*(floor(log10(real(c%nz/c%nipz)))+2),4) )
        gloc = iz_file
        call h5_opencreategroup( group_dist, char(gloc), group, e(2), int(8*(floor(log10(8.))+2),4) )
