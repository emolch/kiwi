...
program test_io
    ...
    deltat = 0.5
    toffset = 0.
    call writeseismogram("test.table", "*", strip1%data, toffset, deltat, nerr )
    if (nerr /= 0) then
        call test_fail("write 1")
    end if
    
    call readseismogram("test.table", "*", seismogram, toffset, deltat, nerr )
    if (nerr /= 0) then
...
!     end if 
    ...
    call writeseismogram("test.mseed", "*", strip1%data, toffset, deltat, nerr )
    if (nerr /= 0) then
        call test_fail("write 3")
    end if
    
    call readseismogram("test.mseed", "*", seismogram, toffset, deltat, nerr )
    if (nerr /= 0) then
